...
- **`src/app.rs`** — `App` state + `Mode` enum (Browse, Compose, NewSessionAgent, ConfirmDelete). Owns `Box<dyn SessionManager>` for testability. Sub-structs defined in-file: `StatusDetector` (pane content change tracking + hysteresis for Codex sessions), `ActivityDetector` (pane_activity timestamp-based status for Claude sessions), `TaskTimers` (elapsed-time tracking), `PreviewState` (preview content/scroll/cache), `BackgroundRefreshState` (async message/stats/conversation refresh polling), `ComposeState` (multi-line text input with cursor), `ConversationBuffer` (parsed JSONL conversation entries per session).
...
- **`src/tmux_control.rs`** — `ControlModeSessionManager` impl using a persistent `tmux -C` pipe instead of spawning subprocesses. `TmuxControlConnection` manages the child process, a background reader task, and FIFO command-response correlation via `VecDeque<PendingCommand>`. Falls back to `TmuxSessionManager` if control mode fails to connect. Also has `decode_octal_escapes()` for control mode's byte-level octal encoding and `quote_tmux_arg()` for shell-style argument quoting.
...
- **`src/logs.rs`** — Claude Code log file reader. Traces tmux pane PID → `lsof` → `.claude/tasks/<uuid>/` → JSONL log file. Extracts last assistant message per session. Also provides `SessionStats` + `update_session_stats()` for incremental JSONL metrics (tokens, cost, tool calls, files touched). Also provides `ConversationEntry` enum + `parse_conversation_entries()` for structured conversation parsing from JSONL (user messages, assistant text, tool use, tool results).
...
- **Dual status detection**: Claude sessions use `ActivityDetector` (pane_activity timestamps from `batch_pane_status()`): activity change → Running, unchanged >6s → Idle, supplemented by JSONL `task_elapsed()` for shorter idle threshold (3s when log says task complete). Codex sessions use `StatusDetector` (pane content change tracking + hysteresis): `normalize_capture()` strips braille spinners, spinner glyphs, digits, arrows, ANSI escapes. **Hysteresis** debounces both directions: Running→Idle requires 30 consecutive unchanged refreshes; Idle→Running requires 5 consecutive changed refreshes. **Exited debounce**: 3 ticks normally, extended to 15 ticks when `active_subagents > 0`. `StatusDetector::prune()` and `TaskTimers::prune()` use `HashSet<&String>` for O(1) cleanup.
...
- **Compose mode**: `Mode::Compose` replaces the old Attached mode. Instead of forwarding individual keystrokes, the user types a full message in a multi-line input area (with cursor movement, newlines, backspace). `Ctrl+Enter` or `Alt+Enter` sends the message via `send_keys_literal()` + `send_keys("Enter")`. `Esc` or `Ctrl+C` cancels. `Ctrl+C` in Browse mode sends an interrupt to the selected session. The compose input renders below the conversation view.
- **Conversation preview**: Claude sessions with a resolved UUID render a structured conversation view from JSONL logs instead of raw `capture-pane` output. `ConversationBuffer` (max 500 entries) stores parsed `ConversationEntry` items per session. `render_conversation()` in `ui.rs` styles entries: bold cyan `> ` prefix for user messages, normal text for assistant, dim indented for tool use/results. Falls back to capture-pane for Codex sessions or Claude sessions without conversation data.
- **Last message display**: Sidebar shows the last Claude assistant message per session (dimmed second line, truncated to 50 chars). UUIDs are resolved via PID→process tree lookup and cached in `App.log_uuids`. Failed UUID resolutions are retried with cooldown to avoid repeated expensive subprocess walks. Messages and conversation entries refresh every 40 ticks (~2s). Only reads incremental JSONL bytes for efficiency.
...
- **Idle capture skip**: Codex sessions idle for 40+ ticks (`IDLE_CAPTURE_SKIP_THRESHOLD`) skip `capture_pane` calls and reuse cached content from `StatusDetector.raw_captures`. Capture resumes if content changes (idle_ticks reset). Claude sessions skip capture entirely — they use conversation preview from JSONL.
- **Batch pane status**: `batch_pane_status()` in `tmux.rs` uses a single `tmux list-panes -a -F "#{session_name} #{pane_dead} #{pane_activity}"` call to get dead status and activity timestamps for all sessions at once. Used by `ActivityDetector` for Claude session status and by `refresh_sessions()` for dead-pane detection. Replaces the older `batch_dead_panes()` approach.
...
- **Async I/O**: All tmux subprocess calls use `tokio::process::Command` instead of `std::process::Command` to avoid blocking the event loop. Browse-mode key navigation uses `refresh_preview_from_cache()` for instant feedback; tick handler calls `refresh_preview()` for periodic updates.
...
- Keep the UI event tick responsive (50ms) with a 4-tick (~200ms) session refresh cadence. Conversation preview from JSONL is synchronous (no subprocess needed).
...
- **Background task cadence**: Messages/conversations/stats refresh every 40 ticks (~2s at 50ms tick rate) via `message_tick % 40`.
