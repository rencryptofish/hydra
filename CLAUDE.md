...
- **`Arc<StateSnapshot>` eliminates double-clone**: Backend clones fields into `StateSnapshot` once, wraps in `Arc`, sends via `watch`. UI swaps the `Arc` pointer (cheap) instead of cloning all fields out of the snapshot. Accessors go through `self.snapshot.sessions`, `self.snapshot.last_messages`, etc.
